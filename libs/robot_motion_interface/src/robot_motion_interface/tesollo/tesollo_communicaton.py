...
class TesolloCommunication:
    ...
    def write_registers(self, address, values):
        self.client.write_registers(
            address=address, values=values, device_id=self.device_id)
    ...
    def set_position(self, position: list[float]):
        ...
        with self.lock:
            ...
            self.client.write_registers(
                address=72, values=intPosion, device_id=self.device_id)
    ...
    def set_pgain(self, pGain: list[int]):
        ...
        self.client.write_registers(address=Delto3FHoldingRegisters.MOTOR1_PGAIN.value,
                                    values=pGain, device_id=self.device_id)
    ...
    def set_free(self, isFree: bool):
        ...
        self.client.write_coil(
            address=Delto3FCoils.JOINT_CUSTOM_MODE.value, value=isFree, device_id=self.device_id)
    ...
    def set_step(self, step):
        ...
        self.client.write_register(address=Delto3FHoldingRegisters.MOTION_STEP.value,
                                   value=step, device_id=self.device_id)
    ...
    def fix_position(self, position: list):
        ...
        with self.lock:
            # print(position)
            self.client.write_registers(
                address=Delto3FHoldingRegisters.MOTOR1_FIXED_POSITION.value, values=position, device_id=self.device_id)
    ...
    def load_pose(self, pose_index: int):
        ...
        with self.lock:
            self.client.write_register(
                address=Delto3FHoldingRegisters.MOTION_LOAD.value, value=pose_index, device_id=self.device_id)
    ...
    def save_pose(self, pose_index: int):
        ...
        with self.lock:
            self.client.write_register(
                address=Delto3FHoldingRegisters.SAVE_TARGET_POSE.value, value=pose_index, device_id=self.device_id)
